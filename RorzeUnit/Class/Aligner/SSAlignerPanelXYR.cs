...
namespace RorzeUnit.Class.Aligner
{
    public class SSAlignerPanelXYR : I_Aligner
    {
        ...
        public void AlgnW(int nTimeout, int nMode = 0, int nPos = -1)
        {
            ...
            if (!m_bSimulate)
            {
                ...
                if (rawGtlcResult.Length == 0)
                    throw new SException((int)enumAlignerError.ProgramError, string.Format("Get GTLC data length = 0.[{0}]", "ALGN"));

                string[] GtlcResult = rawGtlcResult.Split(',');

                if (GtlcResult.Length == 0 || GtlcResult.Length != 4)
                    throw new SException((int)enumAlignerError.ProgramError, string.Format("Get GTLC data format error.", "ALGN"));
                int NotchLocation = int.Parse(GtlcResult[0]);
                int Shift_X = int.Parse(GtlcResult[1]);
                int Shift_Y = int.Parse(GtlcResult[2]);
                int Angle = int.Parse(GtlcResult[3]);
                WriteLog(" GTLC:" + rawGtlcResult);
                m_Motion.ResetInPos();
                m_Motion.Mabs(m_nMotionTimeout, Angle, Shift_Y, Shift_X);
                m_Motion.WaitInPos(m_nMotionTimeout);
            }
            else
            {
        ...
        public virtual void RotationExtdW(int nTimeout, int nPos)
        {
            ...
            if (!m_bSimulate)
            {
                ...
                try
                {
                    ...
                    //WriteLog(" RotationExtdW MotionCompleted Set");
                    m_Signals[enumAlignerSignalTable.MotionCompleted].Set();
                }
                catch (SException ex)
                {
        ...
        public void ResetW(int nTimeout, int nReset = 0)
        {
            ...
            if (!m_bSimulate)
            {
                m_Motion.Reset(m_nMotionTimeout);
            }
            else
            {
