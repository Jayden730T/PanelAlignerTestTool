...
namespace RorzeUnit.Class.Camera
{
    public class SSCamera
    {
        ...
        private SInterruptOneThreadINT m_threadCllc;
        private SInterruptOneThread m_threadGtlc;
        ...
        #endregion
        #region =========================== property ===========================================
        public virtual bool _Connected { get; private set; }
        ...
        public SSCamera(string strIP, int nPort, int nBodyNo, bool bDisable, bool bSimulate, sServer Sever = null)
        {
            ...
            m_threadCllc = new SInterruptOneThreadINT(ExeCllc);
            m_threadGtlc = new SInterruptOneThread(ExeGtlc);
        ...
        public void CLLC(int CllcNo) { m_threadCllc.Set(CllcNo); }
        private void ExeCllc(int CllcNo)
        {
            try
            {
                ...
                this.ResetInPos();
                this.CllcW(m_nAckTimeout, CllcNo);
                this.WaitInPos(m_nAckTimeout);
        ...
        private void CommandW(int nTimeout, enumCommand eCommand, params object[] args)
        {
            ...
            if (_Connected)
            {
                ...
                if (!m_SignalAck[eCommand].WaitOne(nTimeout))
                {
                    ...
                    throw new SException((int)enumCustomError.SendCommandAckTimeout, string.Format("Send command and wait Ack was timeout. [{0}]", eCommand));
                }
                if (m_SignalAck[eCommand].bAbnormalTerminal)
                {
                    SendAlmMsg(enumCustomError.SendCommandFailure);
                    throw new SException((int)enumCustomError.SendCommandFailure, string.Format("Send command and wait Ack was failure. [{0}]", eCommand));
                }
            }
            else
            {
        ...
        public void CllcW(int nTimeout,int CllcNo) { CommandW(nTimeout, enumCommand.CLLC); }
        public void GtlcW(int nTimeout) { CommandW(nTimeout, enumCommand.GTLC); }
        ...
        public void WaitChangeModeCompleted(int nTimeout)
        {
            if (!m_bSimulate)
            {
                if (!m_Signals[enumSignalTable.Remote].WaitOne(nTimeout))
                {
                    ...
                    throw new SException((int)enumCustomError.InitialTimeout, string.Format("Wait initial flag was timeout. [Timeout = {0} ms]", nTimeout));
                }
                if (m_Signals[enumSignalTable.Remote].bAbnormalTerminal)
                {
                    SendAlmMsg(enumCustomError.InitialFailure);
                    throw new SException((int)enumCustomError.InitialFailure, string.Format("Initial failure."));
                }
            }
        }
        ...
        public void WaitOrgnCompleted(int nTimeout)
        {
            if (!m_bSimulate)
            {
                if (!m_Signals[enumSignalTable.OriginCompleted].WaitOne(nTimeout))
                {
                    ...
                    throw new SException((int)(enumCustomError.OriginTimeout), string.Format("Wait origin flag was timeout. [Timeout = {0} ms]", nTimeout));
                }
                if (m_Signals[enumSignalTable.OriginCompleted].bAbnormalTerminal)
                {
                    SendAlmMsg(enumCustomError.OriginFailure);
                    throw new SException((int)(enumCustomError.OriginFailure), "Origin Failure");
                }
            }
            else
            {
        ...
        public void WaitInPos(int nTimeout)
        {
            ...
            if (!m_bSimulate)
            {
                if (!m_Signals[enumSignalTable.MotionCompleted].WaitOne(nTimeout))
                {
                    ...
                    throw new SException((int)enumCustomError.MotionTimeout, string.Format("Wait calculation complete was timeout. [Timeout = {0} ms]", nTimeout));
                }
                if (m_Signals[enumSignalTable.MotionCompleted].bAbnormalTerminal)
                {
                    SendAlmMsg(enumCustomError.MotionAbnormal);
                    throw new SException((int)enumCustomError.MotionAbnormal, string.Format("Calculation Failure."));
                }
            }
            else
            {
